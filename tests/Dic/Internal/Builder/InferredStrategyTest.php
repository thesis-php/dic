...
final class InferredStrategyTest
{
    #[Test]
    public function resolvesToSingletonWithoutDependencies(): void
    {
        $resolution = LifetimeStrategy::Inferred->resolve(ref(), self::edges([]));

        Assert::same($resolution->strategy, LifetimeStrategy::Inferred);
    ...
    #[Test]
    public function recordsThePathDescendingThroughANestedCarrier(): void
    {
    ...
    public function stopsAtTheFirstScopedDependency(): void
    {
        ...
        $resolution = LifetimeStrategy::Inferred->resolve(ref(), $dependencies);
